...
interface Props {
  ...
  userRole:       string;
  lang?:          "pl" | "uk";
  onLangChange?:  (l: "pl" | "uk") => void;
  onMenuToggle?:  () => void;
}

export function TopBar({ title, alerts = [], userName, lang = "pl", onLangChange, onMenuToggle }: Props) {
  ...
  return (
    <header className="h-16 border-b border-white/8 flex items-center gap-4 px-5 sticky top-0 z-30"
      ...
      {/* Title */}
      {title && (
        <h2 className="text-sm font-bold text-fg-muted hidden sm:block flex-shrink-0">{title}</h2>
      )}
            ...
            <>
              ...
              <div className="absolute right-0 top-11 w-80 z-20 rounded-2xl border border-white/10 shadow-2xl overflow-hidden"
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/8">
                  <p className="text-sm font-bold text-fg">Powiadomienia</p>
                  {unread > 0 && <span className="text-xs text-fg-faint">{unread} nieprzeczytanych</span>}
                </div>
                <div className="max-h-72 overflow-y-auto divide-y divide-white/6">
                  {alerts.length === 0 ? (
                    <p className="p-4 text-sm text-fg-muted text-center">Brak powiadomień</p>
                  ) : alerts.map(a => (
